...
func TestMergeSort(t *testing.T) {
	cases := []struct {
		...
		expected: 590,
		input: []int{37, 7, 2, 14, 35, 47, 10, 24, 44, 17, 34, 11, 16, 48, 1, 39, 6, 33,
			43, 26, 40, 4, 28, 5, 38, 41, 42, 12, 13, 21, 29, 18, 3, 19, 0, 32,
			46, 27, 31, 25, 15, 36, 20, 8, 9, 49, 22, 23, 30, 45},
	}, {
		expected: 2372,
		input: []int{4, 80, 70, 23, 9, 60, 68, 27, 66, 78, 12, 40, 52, 53, 44, 8, 49, 28,
	...
	for _, c := range cases {
		_, invs := mergeSort(c.input)
		if invs != c.expected {
			t.Errorf("Expected %v, got %v", c.expected, invs)
		}
	}
}
